...
public class OkxRepo(HttpClient client) : BaseExchangeRepo(client)
{
    public const string BaseAddress = "https://www.okx.com";
    public const string Name = "OKX";
    ...
    public override async Task<IEnumerable<Candle>> GetCandlesAsync(string symbol, DateTime start, DateTime end, string interval, CancellationToken ct = default)
    {
        ...
        {
            RequestUri = new Uri($"/api/v5/market/history-candles?instId={symbol}&before={startUnixTime}&after={endUnixTime}&bar={interval}&limit=300", UriKind.Relative),
